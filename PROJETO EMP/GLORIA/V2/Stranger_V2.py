    ...
    while True:
        try:
            ...
            if localizacao:
                ...
                pyautogui.leftClick(centro.x, centro.y)
                time.sleep(1)
                return True  # Sai do loop após encontrar e clicar no botão de ataque
            else:
                print("Botão de ataque não encontrado. Continuando a busca...")
                time.sleep(1)  # Pequena pausa antes de tentar novamente
        except Exception as e:
            print(f"Erro ao localizar o botão de ataque: {e}")

def localizar_e_fechar_x(region=None, expected_color=0xFFFFFF, tolerance=100):
    """
    Localiza e clica no "X" branco (ou cor próxima) para fechar páginas aleatórias.
    :param region: Região da tela para procurar o "X" (x, y, largura, altura).
    :param expected_color: Cor esperada no formato hexadecimal (ex: 0xFFFFFF).
    :param tolerance: Tolerância para variação de cor.
    """
    print("Procurando o 'X' para fechar páginas aleatórias...")
    try:
        # Converte a cor hexadecimal para RGB usando a função convert_color
        expected_rgb = convert_color(expected_color)
        
        for x in range(region[0], region[0] + region[2]):
            for y in range(region[1], region[1] + region[3]):
                current_color = pyautogui.pixel(x, y)
                if all(abs(c1 - c2) <= tolerance for c1, c2 in zip(current_color, expected_rgb)):
                    print(f"'X' encontrado em ({x}, {y}). Clicando para fechar...")
                    time.sleep(1)
                    ...
                    for castelo in castelos:
                        ...
                        if localizar_botao_ataque("botao_ataque.png", region=(268, 188, 1755, 859)):
                            try:
                                ...
                                localizar_e_fechar_x(region=(1209, 442, 1388, 273), expected_color=0xEBEBE8, tolerance=100)
                        else:
